...
PROTO(PRIVATE void report_mismatch,( Token *term1, Token *operator, Token *term2 ));
PROTO(PRIVATE void report_type,( Token *t ));
...
void
#if HAVE_STDC
binexpr_type(Token *term1, Token *operator, Token *term2, Token *result)
#else /* K&R style */
...
#endif /* HAVE_STDC */
{
	...
	if( (type1 != E && type2 != E) ) {
		...
		report_mismatch(term1,operator,term2);
		  ...
		  op == tok_concat && !in_assignment_stmt &&
...
void
#if HAVE_STDC
unexpr_type(Token *operator, Token *term1, Token *result)
#else /* K&R style */
...
#endif /* HAVE_STDC */
{
	...
	if( (type1 != E && type2 != E) ) {
		...
		result_type -= W;
	      ...
	      if(trunc_demotion || trunc_promotion ||
...
(void)fprintf(list_fd,"%s type %s size %s mixed",
	...
	TorF(mixed_size));
		...
		trunc_warn = (trunc_promotion && promotion) ||
			     (trunc_demotion && type_trunc) ||
			     (trunc_demotion && size_trunc);
...
void
#if HAVE_STDC
stmt_fun_arg_cmp(Lsymtab *symt, Token *d_arg, Token *a_arg)
#else /* K&R style */
...
PRIVATE void
#if HAVE_STDC
report_mismatch(Token *term1, Token *operator, Token *term2)
#else /* K&R style */
report_mismatch(term1,operator,term2)
